...
class _ApprovalAdminPageState extends ConsumerState<ApprovalAdminPage> {
  ...
  static const destinations = [
    ...
    (Icons.folder_shared_outlined, '문서함 권한'),
                            ...
                            child: Center(
                              child: ConstrainedBox(
                                ...
                                child: switch (selectedIndex) {
                                  ...
                                  1 => _DocumentAccessManagement(state: state),
                                  2 => _EmployeeManagement(state: state),
                                  3 => _OrganizationManagement(state: state),
                                  4 => _AppManagement(state: state),
                                  _ =>
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return Column(
      ...
      children: [
        ...
        _AdminDocumentManagement(state: state),
        SizedBox(height: mobile ? 22 : 28),
        Text(
          '휴가 승인 관리',
          style: mobile
...
class _DocumentAccessManagement extends ConsumerStatefulWidget {
  const _DocumentAccessManagement({required this.state});

  final ApprovalDashboardState state;

  @override
  ConsumerState<_DocumentAccessManagement> createState() =>
      _DocumentAccessManagementState();
}

class _DocumentAccessManagementState
    extends ConsumerState<_DocumentAccessManagement> {
  String selectedCategory = '지원';
  ...
  void _save() {
    ...
    for (final category in const ['지원', '회계', '근태', '급여']) {
      notifier.updateDocumentCategoryAccess(
        category: category,
        organizationWide: draftWideCategories.contains(category),
        userIds: draftViewerIds[category] ?? const <String>{},
      );
    }
    showTheWeSnackBar(context, message: '문서함 접근 권한을 저장했습니다.');
  }
        ...
        children: [
          Expanded(
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text('문서함 접근 권한 관리', style: TheWeTextStyle.title),
                const SizedBox(height: 6),
                Text(
                  '결재 구분별로 문서함을 열람할 사용자를 설정합니다.',
      ...
      LayoutBuilder(
        builder: (context, constraints) {
          ...
          final accessPanel = _DocumentCategoryAccessPanel(
            ...
            organizationWide: draftWideCategories.contains(selectedCategory),
            selectedUserIds:
...
class _AdminDocumentManagement extends StatefulWidget {
  const _AdminDocumentManagement({required this.state});
...
class _AdminDocumentManagementState extends State<_AdminDocumentManagement> {
  ...
  Widget build(BuildContext context) {
    ...
    final documents = widget.state.hasAdminDocumentAccess
        ? widget.state.documents
        : widget.state.visibleDocuments;
    ...
    final categoryFiltered = selectedCategory == '전체'
        ...
        : sorted
              .where(
                (document) =>
                    widget.state.documentCategory(document) == selectedCategory,
              )
              .toList();
    ...
    final categories = ['전체', '지원', '회계', '근태', '급여'];

    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Text('결재 구분', style: TheWeTextStyle.subtitle),
        const SizedBox(height: 8),
        Wrap(
          spacing: 8,
          runSpacing: 8,
          children: categories.map((category) {
            final count = category == '전체'
                ...
                : sorted
                      .where(
                        (document) =>
                            widget.state.documentCategory(document) == category,
                      )
                      .length;
            return ChoiceChip(
              key: ValueKey('admin-document-category-$category'),
        ...
        Text('문서 상태', style: TheWeTextStyle.subtitle),
        const SizedBox(height: 8),
        Wrap(
          spacing: 8,
          runSpacing: 8,
        ...
        Container(
          ...
          child: visible.isEmpty
              ...
              ? Column(
                  children: visible
                      .map((document) => _AdminDocumentCard(document: document))
                      .toList(),
                )
              : TheWeDataTable(
                  headers: const [
                  ...
                  rows: visible
                      .map(
                        (document) => <Widget>[
                          ...
                          OutlinedButton(
                            onPressed: () =>
                                _openAdminDocument(context, document),
                            style: OutlinedButton.styleFrom(
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Text('${widget.category} 문서함 접근 권한', style: TheWeTextStyle.subtitle),
          const SizedBox(height: 8),
          Wrap(
            spacing: 18,
            runSpacing: 4,
